...
var doLog = function doLog() {
    process.stderr.write("pyret-kernel: ");
...
if (process.env.DEBUG) {
    ...
    try {
        doLog = require("debug")("pyret-kernel:");
    } catch (err) {}
}
 ...
 * @property {String}   context.path.root     Path to pyret-kernel root folder
 * @property {String}   context.path.kernel   Path to pyret-kernel kernel
 * @property {String}   context.path.images   Path to pyret-kernel images folder
...
var FLAGS = [{
    ...
    flag: "help",
    description: "show pyret-kernel and Jupyter/IPython help",
    ...
    flag: "version",
    description: "show pyret-kernel version",
    parse: function(context, arg) {
    ...
    prefixedFlag: "help",
    description: "show pyret-kernel help",
    parse: function(context, arg) {
    },
    showUsage: true,
    exit: true,
    ...
    description: "install pyret-kernel kernel",
    parse: function(context, arg) {
...
function setProtocol(context) {
    ...
    if (context.frontend.majorVersion < 3) {
        ...
        context.args.frontend.push("--kernel=pyret");
    }
...
function installKernelAsync(context, callback) {
    ...
    var specDir = path.join(tmpdir, "pyret");
